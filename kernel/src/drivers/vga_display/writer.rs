...
impl<const W: usize, const H: usize> const Default for Writer<W, H> {
    fn default() -> Self {
        Self {
            ...
            screen: unsafe {
                core::slice::from_raw_parts_mut(VGA_BUFFER_PTR as *mut ScreenChar, W * H + 1)
            },
        }
    }
}
...
impl<const W: usize, const H: usize> Writer<W, H> {
    ...
    fn write_char(&mut self, char: u8) {
        // ANCHOR: handle_char
        let c = Char::from_u8(char).expect("Entered invalid ascii character");
        match c {
            Char::LineFeed => {
                self.new_line();
            }
            ...
            _ => {
                if !c.is_control() {
                    self.screen[self.cursor_position] = ScreenChar::new(char, self.color);
